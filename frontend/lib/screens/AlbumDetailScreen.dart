...
class _AlbumDetailScreenState extends State<AlbumDetailScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final albumRes = await http.get(
        Uri.parse('$apiUrl/api/albums/${widget.albumId}'),
      );
        ...
        await Future.wait([
          if (artistId != null)
            http
                ...
                .then((res) {
                  if (mounted)
                    setState(() => artistAlbums = json.decode(res.body));
                }),
          if (genreId != null)
            http
                .get(
                ...
                .then((res) {
                  if (mounted)
                    setState(() => genreAlbums = json.decode(res.body));
                }),
        ]);
      }
    } catch (e) {
  ...
  Future<void> _fetchComments() async {
    ...
    try {
      final res = await http.get(
        Uri.parse('${auth.apiUrl}/api/comments/album/${widget.albumId}'),
      );
  ...
  Widget build(BuildContext context) {
    ...
    if (currentAlbum['genreID'] is Map) {
      ...
      final genreData = genres.firstWhere(
        (g) => g['_id'] == currentAlbum['genreID'],
        orElse: () => null,
      );
            ...
            Expanded(
              child: SingleChildScrollView(
                padding: EdgeInsets.symmetric(
                  horizontal: isDesktop ? screenWidth * 0.12 : 20,
                  vertical: 30,
                ),
                child: Column(
                  ...
                  children: [
                    isDesktop
                        ? _buildDesktopHeader(currentAlbum, genreName)
                        : _buildMobileHeader(currentAlbum, genreName),
    ...
    return Column(
      ...
      children: [
        ...
        Wrap(
          ...
          children: [
            // Genre clickable (giống artist)
            InkWell(
              onTap: () {
                final genreData = data['genreID'];

                String? genreId;
                String genreNameLocal = genreName;

                if (genreData is Map) {
                  genreId = genreData['_id']?.toString();
                  genreNameLocal = genreData['name'] ?? genreName;
                } else {
                  genreId = genreData?.toString();
                }

                if (genreId == null || genreId.isEmpty) {
                  print("❌ genreId NULL → KHÔNG NAVIGATE");
                  return;
                }

                print("✅ NAVIGATE GENRE: $genreId");

                Navigator.pushNamed(
                  context,
                  '/genre-detail',
                  arguments: {'genreId': genreId, 'genreName': genreNameLocal},
                );
              },
              child: _buildBadge(
                Icons.label,
        ...
        Text(
          '${NumberFormat('#,###').format(data['price'])} ${data['currency'] ?? 'VND'}',
      ...
      child: Column(
        ...
        children: [
          ...
          Padding(
            ...
            child: TextButton(
              onPressed: () =>
                  setState(() => _showReply[id] = !_showReply[id]!),
              child: Text(
                _showReply[id]! ? 'Cancel' : 'Reply',
              ...
              child: Row(
                children: [
                  ...
                  IconButton(
                    ...
                    onPressed: () => _submitComment(parentId: id),
                  ),
                ],
              ),
            ),
            ...
            Row(
              children: List.generate(
                ...
                (i) => Icon(
                  Icons.star,
                  size: 12,
                  color: i < (data['rating'] ?? 5) ? Colors.amber : Colors.grey,
                ),
              ),
            ),
